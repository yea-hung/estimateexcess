...
estimate_weekly_excess<-function(yy,forecast.window=143,
  ...
  # define prior deaths
  pp<-data
  earliest.year<-min(as.numeric(substr(pp$week,1,4)))
  pp<-lapply(earliest.year:2019,function(year){
    prior.start<-as.Date(paste(year,'03-07',sep='-'),'%Y-%m-%d')
    prior<-subset(pp,week>=prior.start)
    prior<-prior[1:forecast.window,c('week',yy)]
    prior<-subset(prior,week<as.Date('2020-03-07','%Y-%m-%d'))
    names(prior)[2]<-'prior'
    prior$year<-year
    prior$week<-prior$week+(2020-year)*365.25
    prior
  })
  pp<-do.call(rbind,pp)
  ...
  x.major<-rr[seq(1,length(rr),24)]
  x.minor<-rr
  # define plot
  PP<-ggplot(aes(x=week,y=observed),data=oo)+
    geom_line(aes(x=week,y=prior,group=year),data=pp,color='gray85')+
    geom_ribbon(aes(x=week,y=expected,ymin=expected.lower,
                    ymax=expected.upper),data=ee,
                alpha=0.2,fill='#00BFC4')+
    geom_line(aes(x=week,y=expected),data=ee,color='#00BFC4')+
